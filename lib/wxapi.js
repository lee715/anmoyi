...
(function() {
  ...
  API = (function() {
    ...
    API.prototype.recharge = function(req, callback) {
      var _placeId, money, openid;
      if (!(money && openid && _placeId)) {
      ...
      return db.order.createAsync({
        ...
        return WX_API.getBrandWCPayRequestParamsAsync(openid, "" + order._id, money * 100).then(function(args) {
    ...
    API.prototype.recharge.route = ['get', '/pay/v1/recharge'];

    API.prototype.pageView = function(req, callback) {
      var _placeId, info, openid;
      openid = req.query.openid;
      _placeId = req.query._placeId;
      info = {};
      return db.place.findOneAsync({
        ...
        info.devices = _.map(devices, function(device) {
        ...
        return req.res.render('payWithPlace', info);
      })["catch"](function() {
        return req.res.send('system error, please try later');
      });
    };

    API.prototype.pageView.route = ['get', '/pay/v1/h5page'];

    API.prototype.wxExcharge = function(req, callback) {
      var _orderId, alien, openid, order, ref;
      ref = req.query, _orderId = ref._orderId, openid = ref.openid;
      if (!(openid && _orderId)) {
    ...
    API.prototype.wxExcharge.route = ['get', '/wx/excharge'];
