  ...
  API = (function() {
    ...
    API.prototype.reconciliation = function(req, callback) {
      ...
      return db.place.findAsync({
        ...
        var _agentId, months, ref1;
        _agentId = place._agentId;
        if (user.role === 'agent' && ("" + place._agentId) === ("" + user._id) || ((ref1 = user.role) === 'place' || ref1 === 'root')) {
          return months = [moment().startOf('month'), moment().startOf('month').add(-1, 'month'), moment().startOf('month').add(-2, 'month'), moment().startOf('month').add(-3, 'month')];
        } else {
          return [];
        }
      }).map(function(start) {
