...
(function() {
  var API, MP_API, WX_API, _, async, config, db, deviceJson, formatUser, pluck, sockSrv, u, wxReply;
  ...
  API = (function() {
    ...
    API.prototype.orderDevice = function(req, callback) {
      ...
      return db.order.findOneAsync({
        ...
        if (action === "start") {
          return sockSrv.start(uid, order.time, function(err, rt) {
            return console.log('orderDevice:start', err, rt);
          });
        } else if (action === '1F' || action === '20' || action === '1E' || action === '21' || action === '22' || action === '24') {
          return sockSrv.set(uid, action, function(err, rt) {
            console.log('orderDevice:set', err, rt);
            return callback(null, "ok");
          });
        } else {
          return callback(new Error('unknownAction'));
        }
      });
    };
