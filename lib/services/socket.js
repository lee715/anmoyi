...
(function() {
  var PORT, Promise, SERVICES, SOCKS, STATUS_VALS, TbService, _, apis, config, createTbOrder, db, moment, net, record, recordStatus, redis, updateDevice;
  ...
  createTbOrder = function(uid, total) {
    return db.device.findOneAsync({
      uid: uid
    }).then(function(device) {
  ...
  record = function(uid) {
    ...
    return redis.incr("RECORD." + uid + "." + key);
  };
  ...
  SOCKS = {
    ...
    handleCB: function(uid, callback) {
      if (!this._cbs[uid]) {
        this._cbs[uid] = [];
        this._cbs[uid].cb = (function(_this) {
          return function(state, err) {
            var cb;
            console.log('cb called');
            cb = _this._cbs[uid].shift();
            cb(null, !!state);
            console.log(_this._cbs[uid].length);
            if (!_this._cbs[uid].length) {
              return delete _this._cbs[uid];
            }
          };
        })(this);
      }
      return this._cbs[uid].push(callback);
    },
    ...
    handleMsg: function(msg, sock) {
      ...
      uid = arr[0];
      if (this._cbs[uid]) {
        if (arr.length === 4 && arr[3] === 'OK') {
          this._cbs[uid].cb(true);
        } else if (arr.length === 4 && arr[3] !== 'OK') {
          this._cbs[uid].cb(false, arr[1]);
        } else {
          this._cbs[uid].cb(false, 'NO_ANWSER');
        }
      }
      if (arr.length === 3) {
